...
A skill for authoring and iteratively improving local InFu skills.
...
InFu discovers skills in these directories (highest priority first):

- `~/.infu/skills/<name>/SKILL.md`（用户级，跨项目）
- `<project>/.infu/skills/<name>/SKILL.md`（项目级）
- config `skills[]` 显式引用
- 内置技能（随 InFu 分发，最低优先级）

**Default to creating new skills under `~/.infu/skills/`**（用户级，跨项目可用）或 `<project>/.infu/skills/`（仅当前项目）。用户级目录同名优先于项目级。写技能也可直接写到 `<project>/.infu/skills/<name>/SKILL.md`，文件系统即注册——无需注册工具。
...
Optional (reserved) frontmatter fields are listed in the InFu skill spec; for most skills you only need `name` and `description`.

### Progressive disclosure

InFu loads skills in three layers (progressive disclosure):
...
Then run them: load the draft skill, hand the model the test prompt, and inspect what happens. InFu can spawn parallel subagents via delegate_task for evaluation, but for skill iteration prefer one prompt at a time and look at each result with the user.
...
1. Make sure the draft skill is on disk where InFu can discover it (one of the directories listed above).
2. In a fresh InFu turn, give the test prompt to the model. Either let the description trigger the skill, or call the `use_skill` tool with the skill name to force-load it.
...
- If the installed skill path is read-only (e.g., a built-in skill shipped under the InFu package), copy the skill to a writable user location like `~/.infu/skills/<name>/`, edit there, and let user-priority discovery override the original.
