...
- 🔄 **v2 按 ROADMAP 7 阶段推进**（v2.1 ✅，v2.2 ✅ 全两批，v2.3 ✅ 全两批，v2.4 ✅ 全两批，v2.5 ✅ 子智能体与并行：opencode 式委派/真并发/agent 文件化定义；best-of-n 按用户评审移除；v2.6 ✅ 批 1 记忆核心 + v2.6.1 会话中枢重构（任务看板按用户评审删除）：INFU.md 指令/四层记忆/路径作用域/自动沉淀/会话管理（隶属·顶置·归档·重命名）/项目注册表/侧栏会话中枢）：下一步 **v2.6 收尾**（Git 优化 + 工具调用优化，视体量可顺延 v2.7）→ **v2.7 生态与数据**。v2.2 遗留仅剩：**完整 codex 式模型选择流程**（细节实施前讨论）；v2.6 遗留：记忆索引/剪枝、后台提炼管道（v2.7）
