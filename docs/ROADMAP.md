...
> 分期原则：地基先于智能，智能先于生态。UI 决策必须先讨论后动手（见 AGENTS.md 项目约定）。

### v2.1 地基期（数据持久化 + 模型适配 + 扩展机制）
- **会话/任务持久化**：多会话、历史浏览、继续会话、断点恢复 + Rewind（会话回滚到检查点）——记忆/任务/统计/审计全部依赖
- **模型适配 + 选择优化**：provider 兼容矩阵实测（GLM/通义/Kimi/Ollama 未实测；reasoning/工具调用/流式差异）+ codex 式模型选择流程（按任务类型路由，细节实施前讨论）+ API 失败自动重试/降级备用模型
- **扩展机制架构**：插件系统 v1（插件=可注册工具/命令/技能/钩子的包）、钩子系统（生命周期事件：PreToolUse/PostToolUse 等，自动化/记忆/权限都依赖）、skill 加载机制（SKILL.md 社区标准兼容）、**MCP 客户端作为第一个插件类型**（`infu mcp add <server>`，工具动态注入 Agent 循环，审批/审计覆盖）
- **配置系统 + 设置界面 v1**：配置结构化 schema；**权限等级设置**（审批模式/按风险/按工具/命令白名单/通配符/禁用工具）+ **沙箱等级设置**（off/L1/L1.5/L2 Docker/自动，取代 INFU_SANDBOX 环境变量）
- **终端功能**（Web 交互式终端）
- 上下文压缩（长会话自动摘要）+ 动态步数（任务复杂度评估）

### v2.2 智能期（能力深化）
- **子智能体**（opencode 式）：主 agent 委派、独立上下文、并行执行、结果回收；**agent 文件化定义**（markdown 定义角色/工具/模型，参考 opencode 的 build/plan/explore 体系）
- **记忆系统三层**（项目记忆/任务总和记忆/全局记忆，形态实施前讨论）+ **路径作用域规则**（不同目录不同规则，CLAUDE.md 式）+ 项目级指令文件（INFU.md）
- **项目任务系统**（任务看板/TODO，agent 可读写可规划）+ 可选 rubric 完成度自评（Claude Code Outcomes 参考）
- **Git 优化**（zcode 参考：diff/暂存/提交/分支 UI）+ **工具调用优化**（并行调用/工具选择/错误自适应）
...
- **定时任务/自动化**：cron + webhook/HTTP 触发源（Claude Code Routines 参考）
- **数据与统计**：成本/用量追踪（token/费用，模型路由决策依据）、审计可视化/任务回放
