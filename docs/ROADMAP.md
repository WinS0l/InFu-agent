...
- **三栏与右栏**——聊天卡片与右栏恢复清晰边界；右栏 tab 条统一背景/分隔/活动表面及新建 Tab 按钮，移除展开态左上角重复“工作区”标签以释放 tab 空间；空态由四个等价按钮升级为分层“工作区”入口卡（审查改动为主入口，浏览器/子 Agent/computer-use 为辅助入口），不改变现有浏览器常驻与 tab 生命周期。
...
### ✅ v6.5 Token 效率、Agent 弹性与会话追踪（2026-08-19）
- **Token 与缓存命中**——工具定义规范按名称排序，扩展注册次序改变不再扰动请求前缀；上下文摘要调用复用正常调用的 tools 前缀并显式 `tool_choice:"none"`，保留上游 KV cache 命中且防止摘要阶段误调工具。
- **任务恢复效率**——模型上下文中的单工具结果上限 8K→6K 字符，采用 head+tail 裁剪保留退出码、异常与最终统计，完整输出仍落会话事件流；上下文窗口 400 仅在强制裁剪/摘要实际降低消息表面后重试，无法缩减则直接给出可行动提示，杜绝无效重试循环。
- **UI**——右栏新增「会话追踪」Tab（空态与新建 Tab 菜单均可打开）：实时 SSE 与历史重放复用同一份按会话原始事件账本，展示模型输入/缓存读/输出、工具、压缩、降级、重试、子 Agent；选中行可查摘要和原始事件 JSON，常规聊天 Timeline 不增加噪声。
- **验证**——全量 **51/51、1545 断言 0 失败**，根 build、web tsc/vite、agent tsc、lint 0 error 通过。Rust 原生模块的 `index.node` 被运行中服务占用时构建保留备用产物，重启服务加载即可。

### ✅ v6.6 会话追踪修复与运行可见性（2026-08-19）
- **白屏修复**——会话追踪在没有账本事件时，Zustand selector 原本每次渲染创建新的 `[]`，React 将其视为状态持续变化并触发无限更新/白屏；改为模块级稳定空数组，空会话可靠显示空态。
- **欢迎态**——保持项目选择与中央 composer 主入口，增加低噪声的本地 Agent 状态、三项能力概览和可点击任务建议，使首屏更明确但不引入冗余导航。
- **会话运行条**——位置定为输入框正上方：由会话事件账本重建未完成 `tool-start`、`job-start`、`subagent-start`，显示当前工具、后台命令和子 Agent 计数；空闲即隐藏，点击打开会话追踪。该位置兼顾即时可见、与后续输入/排队消息相邻，且不污染欢迎页与消息正文。
- **验证**——web tsc + vite build、全量 **51/51、1545 断言 0 失败**。

### ✅ v6.7 悬浮任务胶囊与欢迎字标收敛（2026-08-19）
- **任务胶囊**——运行状态改为固定右下方、可 pointer 拖拽的悬浮胶囊；双击展开菜单查看活动工具/后台命令/子 Agent，并可直达会话追踪；10 秒无交互自动收为圆形任务图标，单击恢复。独立拖拽标记防浏览器拖拽结束后的 click 误触。
- **欢迎页**——移除「INFINITE FUTURE · LOCAL AGENT」、能力卡及建议任务，保留项目选择与 composer；「无限未来」升级为唯一视觉焦点（渐变字面、低频呼吸光、reduced-motion 禁用动画），避免把基础能力说明当作首屏内容。
- **验证**——web tsc + vite build 通过。

### ✅ v6.8 固定会话状态入口（2026-08-19）
- **任务胶囊调整**——用户决定不需要拖拽/自动圆点：改为聊天面板右上角固定常驻（只在非欢迎聊天态显示）；活动显示 spinner 与当前工作，空闲显示「会话待命」，双击菜单可查看详情/直达追踪。
- **欢迎字标**——「无限未来」字距放宽，减少紧凑感。
- **验证**——web tsc + vite build 通过。

### ✅ v6.9 右上任务胶囊拖拽恢复（2026-08-19）
- **交互澄清**——保留右上角默认位置但恢复胶囊拖拽；位置限定于聊天面板，拖动后不误开双击菜单；欢迎页仍隐藏，活动/待命语义不变。
- **验证**——web tsc + vite build 通过。

### ✅ v6.10 任务胶囊紧凑收纳（2026-08-19）
- **最终交互**——保留拖拽；10 秒无交互缩为 32px 圆点，点击或任意交互恢复，双击菜单语义不变。展开胶囊收至 132×32px，去除冗余「双击查看会话状态」，只展示当前工具或「会话待命」。
- **可靠性**——将计时相关 Hooks 移到 `visible` 条件返回前，欢迎/聊天切换不再触发 Hook 顺序违规。
- **验证**——web tsc + vite build 通过。

### ✅ v6.11 会话胶囊状态面板（2026-08-19）
- **会话隔离确认**——胶囊从 `activeSessionId` 精确选择 `traceBySession[sessionId]`，切换会话后读取各自持久/实时事件账本，不复用上个会话工具、后台命令或子 Agent。
- **展开态优化**——状态面板显示当前会话标题、待命/运行总数、工具/后台/Agent 分类徽标及逐项名称+类型，空闲时说明无活动工作；切换会话立即关闭旧展开态，避免状态错读，底部保留完整事件账本入口。
- **验证**——web tsc + vite build 通过。

### ✅ v6.12 胶囊内容宽度收敛（2026-08-19）
- **视觉收敛**——展开态删除固定 `132px` 宽度，改 inline-flex 按图标与文本自然包裹；文字最大 112px 后截断，避免长工具名撑大且没有无意义空白。
- **验证**——web tsc + vite build 通过。

### ✅ v6.13 胶囊状态菜单信息重组（2026-08-19）
- **菜单内容**——顶栏仅状态图标与会话标题，无分隔横线；后台命令显示命令与每秒刷新的运行时长；子 Agent 显示名称/委派内容并可点击直达右栏线程；工具列表替换为基于本会话原始工具结果累加的 `+n/-m` Diff 行。
- **显隐规则**——后台/子 Agent/Diff 任一无数据即不渲染，全无时显示「目前没有内容」；「打开会话追踪」为唯一常驻操作项。
- **验证**——web tsc + vite build 通过。

### ✅ v6.14 工作树控制核查与侧栏运行环（2026-08-19）
- **工作树核查**——确认开启工作树后，最近修改文件的 AI 消息 turn 尾渲染并入主分支/丢弃工作树两个按钮；分别接 `POST /api/worktree/:name/merge` 和 `discard`，CodeView/ReviewPane 也在模式开启时读取 worktree root，功能链路完整。
- **侧栏状态**——运行中的会话由单绿点改为六段环形横杠 spinner，提高状态辨识，尺寸仍为紧凑 16px。
- **验证**——web tsc + vite build 通过。

### ✅ v6.15 侧栏文本选择收口（2026-08-19）
- **交互修复**——侧栏展开/折叠根节点统一 `select-none`，项目/会话/按钮等反复点击不再选中内部文字；HTML input 自身仍可选择和编辑，不影响搜索/重命名输入。
- **验证**——web tsc + vite build 通过。

### ✅ v6.16 会话胶囊悬浮与摘要轮播（2026-08-19）
- **视觉**——圆点、展开胶囊、状态菜单统一近场阴影/顶部内高光；hover 轻抬，增强浮层质感。
- **信息轮播**——有多类数据时展开胶囊每 3 秒在当前会话的累计 Diff `+n/-m`、后台命令数、子 Agent 数、当前工具之间轮播；用户拖拽/点击/双击重置到首项，reduced-motion 下只禁用入场文字动画。
- **验证**——web tsc + vite build 通过。

### ✅ v6.17 审查/代码会话 root 对齐（2026-08-19）
- **根因与修复**——胶囊从会话历史工具事件累计 `+/-`，审查/代码从当前 Git/文件系统读取，两者语义不同；同时 ReviewPane/CodeView 曾直接取全局 root 并无条件采用持久工作树，跨会话或失效工作树可导致空白。改为优先 active session root，且 worktree 仅当真实位于该项目 `.infu/worktrees/` 才使用。
...
- ✅ **v6.1 审计修复批（2026-08-19，第三方审计 TOP 10 全落地 + 工程化）**：
  - **① 搜索凭据防护**——`SEARCH_SKIP_DIRS`（.ssh/.aws/.gnupg/.docker/.infu 等）走 walkFiles/collectFiles 双通道过滤 + search_code/semantic_search 接线（凭据文件永不出现在索引与搜索结果）
  - **② 图片 token 估算校准**——context.ts `IMAGE_PART_TOKENS=1500` + estimateTokens 显式累加（图片不再按 0 token 计入上下文预算——此前视觉图片对预算系统不可见）
  - **③ 预算守卫缺口**——budgetExhausted 阶段跳过 + 编排跨阶段扣减 + tryAutoRefine 包裹（累计超预算后不再触发额外模型调用；v6.0 S4 只挡主循环）
  - **④ tsserver/持久 shell EPIPE 加固**——lsp 全通道 error 监听 + spawn try/catch（tsserver 崩溃/管道断不再崩宿主）；**persistent-shell 幽灵句柄根治**（真实缺陷，实测复现）：close→重建竞态 = 旧进程 kill 异步退出，其 `exit` 监听 `sessions.delete(sessionId)` 按 **id** 盲删，把随后重建存入的新会话误删 → 残留健康 cmd.exe 永不 kill、Node 事件循环挂死（CLI 任务跑完不退出）；修复 = exit 监听按**会话对象身份**比对删除 + `killSession`（kill+unref+destroy 管道防幽灵句柄）+ sessionAlive 补 `proc.killed`；server 挂 uncaughtException/unhandledRejection 兜底（记日志不退出）
  - **⑤ git 白名单凭据面收窄**——移除 `git remote -v*`/`git config --get*`/`--list*`/`-l*`/`--global --get*`（remote URL 可内嵌令牌、extraheader 可带 Authorization），仅保留 user.name/user.email 读取
  - **⑥ DANGEROUS 运行时绕过**——dangerous.ts 重写：erase 并入删除分支、`git clean -f/--force` 分支、**解释器载荷检测**（INTERPRETER_RUN：node -e / python -c / powershell -Command / -EncodedCommand 等 + `hasDestructiveRuntimePayload`——rmSync/shutil.rmtree/Remove-Item 命中高危，-enc 一律高危）；run_command/run_test/终端三通道统一 `isDangerousCommand`（terminal 套件 +10 断言）
  - **⑦ 受限命令 abort 通道**——Rust 导出 `abort_run(run_id)`（RUN_REGISTRY OnceLock<Mutex<HashMap<u32, usize>>>，napi u32 限制）→ job::terminate；win-restricted runRestricted 接 `AbortSignal`（tool/任务中止可中断沙箱内命令；**本机嵌套 Job 环境 AssignProcessToJobObject 失败 → terminate 杀不到属既有 OS 限制，timeout 兜底保留**）
  - **⑧ 桌面 IPC sender 校验**——`isTrustedSender`（e.sender === 主窗口 webContents）收口全部 19 个 ipcMain handler + session:open（恶意页面/iframe 不可再驱动桌面窗口/浏览器/对话框）
  - **⑨ 并行委派失败隔离**——delegateTasks Promise.all → **allSettled**（单路子 Agent 启动失败不再整体 reject，失败条目占位文本回填）
  - **⑩ 工程化**——新 `smoke-core.test.ts` 22 断言（semantic 中文分词/持久 shell 时序/collectFiles 保护/dangerous 纯函数）+ 根 eslint flat config（0 error，存量 110 warn 不阻塞）+ `.github/workflows/ci.yml`（windows runner：npm ci → build → 三包 tsc → cargo check → lint → 全量测试）
  - **验证**：**51/51 套件全绿 1512 断言 0 失败**（新增 smoke-core 22；terminal 41→51）+ agent/web/desktop tsc + cargo check + lint 0 error 全过。**遗留**：CI 首次推送后实测（本机无 GitHub 仓库）。**顺手修复**：loop/orchestrator tsc 类型（ModelCallEvent time 字段、approvals 类型）、index/index.ts 顶层循环依赖 TDZ（SEARCH_SKIP_DIRS 移函数体内）
- **条件触发项（单独挂起，条件未到不动）**：团队版 v3（第二个真实用户）、microVM 沙箱（多租户/不可信代码）、C5 中英文统一（未定产品方向，不优先）
- **记录不改**：abort 通道已落地但本机嵌套 Job 环境 terminate 杀不到（既有 OS 限制，timeout 兜底）、browser_eval low 免审批（设计权衡）、terminal AttachConsole 环境噪声；**永久不做**：远程插件市场、开机自启（设置项默认关）
