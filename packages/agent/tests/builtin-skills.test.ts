...
check("3 个官方插件", ["browser-use", "document-skills", "skill-creator"].every((id) => builtin.some((b) => b.id === id)), builtin.map((b) => b.id).join(","));
...
check("空配置 → 3 个内置插件", merged.length === 3, JSON.stringify(merged.map((m) => m.id)));
const disabled = mergeBuiltinPlugins([{ id: "browser-use", path: "x", source: "builtin", enabled: false }]);
check("禁用标记生效（browser-use 不加载）", !disabled.some((m) => m.id === "browser-use") && disabled.some((m) => m.id === "document-skills"), JSON.stringify(disabled.map((m) => m.id)));
...
check("插件技能目录 6 个", r.skillDirs.length === 6, JSON.stringify(r.skillDirs.map((d) => d.split(/[\\/]/).pop())));
registerPluginSkillDirs(r.skillDirs);
const skills = listSkills({ models: [] }, "E:/nonexistent-root");
check("docx/pdf/pptx/skill-creator 以 plugin 级挂载", ["docx", "pdf", "pptx", "skill-creator"].every((n) => skills.some((s) => s.name === n && s.level === "plugin")), skills.map((s) => s.name + ":" + s.level).join(","));
