...
console.log("▶ dangerous 运行时载荷");
{
  ...
  check("node -e 无空格载荷命中", isDangerousCommand("node -e\"require('fs').rmSync('x',{recursive:true})\"") === true);
  check("python -c shutil.rmtree 命中", isDangerousCommand("python -c \"import shutil; shutil.rmtree('x')\"") === true);
  check("python -c 无空格载荷命中", isDangerousCommand("python -c\"import shutil; shutil.rmtree('x')\"") === true);
  check("del 无空格开关命中", isDangerousCommand("del/s/q C:\\temp\\x") === true);
  check("cmd /c 内层删除命中", isDangerousCommand("cmd /c del/s/q C:\\temp\\x") === true);
