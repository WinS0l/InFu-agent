...
- 🔄 **v2 按 ROADMAP 7 阶段推进**（v2.1 ✅，v2.2 ✅ 全两批，v2.3 ✅ 全两批：MCP 客户端 + 插件/钩子/skill）：下一步 **v2.4 设置界面与终端**（配置系统 UI 化：**权限等级设置**（审批策略：按风险/工具/命令配置，全自动↔全确认）+ **沙箱等级设置**（off/L1 软/L1.5 受限/L2 Docker/自动，取代 INFU_SANDBOX 环境变量）+ 插件/skill/MCP 管理 UI 并入 + 常规/外观/模型设置 + Web 交互式终端——配置 schema 基础已就位（zod + version + 未知字段保留）。v2.2 遗留仅剩：**完整 codex 式模型选择流程**（细节实施前讨论）
